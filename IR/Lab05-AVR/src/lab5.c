...
void init_directory() {
	...
	if (pf_opendir(&directory, "/music") != FR_OK) {
		return;
	}

	/* TODO3: find the first file whose name doesn't start with a `_`; use the `directory` and `file` structures declared above */
	while (pf_readdir(&directory, &file) && (strncmp("_", file.fname, strlen("_")) == 0 || strcmp(file.fname, "") != 0));
...
void buttons_init() {
	...
	DDRD &= ~(1 << PD6); // Setam pinul 2 al portului D ca input (0)
  	PORTD |= (1 << PD6); // Activam rezistenta de pull-up (PD6 e input)
}
...
int main (void)
{
	...
	for(;;)
	{
		...
		for(;;)
		{
			...
			if (!button2IsNotPressed && button2IsNotPressedLast) {
				char filePath[20] = "";
				sprintf(filePath, "/music/%s", file.fname);
				play(filePath);
			}

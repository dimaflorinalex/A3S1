...
DWORD load_header(void)
{
	...
	for (;;)
	{
		...
		case FCC('f', 'm', 't', ' '):
			...
			if (ret != size || BUF_FRONT[0] != 1)
				return 0;
			// verifica numarul de canale
			if (BUF_FRONT[2] != 1 && BUF_FRONT[2] != 2)
				return 0;
			// verifica rezolutia
			if (BUF_FRONT[14] != 8 && BUF_FRONT[14] != 16)
				return 0;
...
void init_directory()
{
	...
	while ((fr_result = pf_readdir(&directory, &file)) == FR_OK && file.fname[0] == '_')
	{
		;
	}
	...
	if (fr_result != FR_OK)
	{
		LCD_printAt(0x00, "File Read Error");
	}
	else
	{
...
void next_file()
{
	...
	while ((fr_result = pf_readdir(&directory, &file)) == FR_OK && (file.fname[0] == '_' || file.fname[0] == '\0'))
	{
		if (file.fname[0] == '\0')
			pf_readdir(&directory, NULL);
	}
...
int main(void)
{
	...
	for (;;)
	{
		...
		for (;;)
		{
			...
			if (!(PINB & (1 << PB2)) && total_ms > last_button_press + DEBOUNCE_DELAY)
			{
				next_file();
				last_button_press = total_ms;
			}
			/* TODO5: play current file on the speaker when button PD6 is pressed */
			if (!(PIND & (1 << PD6)))
			{
				LCD_clear_top_line();
				LCD_printAt(0x00, "Playing!");

				snprintf(path, 200, "%s/%s", "/music", file.fname);
				play(path);
			}
		}
	}

	...
	if (send_command(CMD0, 0) == 1)
	{
		...
		if (send_command(CMD8, 0x1AA) == 1)
		{
			...
			if (ocr[2] == 0x01 && ocr[3] == 0xAA)
			{
				...
				for (tmr = 12000; tmr && send_command(ACMD41, 1UL << 30); tmr--)
		...
		{ /* SDv1 or MMCv3 */
			...
			for (tmr = 25000; tmr && send_command(cmd, 0); tmr--)
...
DRESULT disk_writep(const BYTE *buf, DWORD sa)
{
	...
	else
	{
		...
		{ /* Initiate sector write process */
			...
			{ /* WRITE_SINGLE_BLOCK */
				SD_transmit(0xFF);
				SD_transmit(0xFE); /* Data block header */
		...
		{ /* Finalize sector write process */
			...
			{ /* Receive data resp and wait for end of write process in timeout of 300ms */
				for (bc = 65000; SD_receive() != 0xFF && bc; bc--)
					; /* Wait ready */
